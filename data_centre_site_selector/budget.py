...
budget.py
   ...
   * What are the hardcoded parameters doing here? How are they chosen?
   * 1 + CONTINGENCY RATE??
   * How do the building materials summaries work?
   * Is the compute given per-centre or total?
   * 
...
def recommended_centre_count(
    compute_mw: float, budget_gbp: float | None, capex_per_mw: float
) -> int:
    technical_count = max(
        1, int((compute_mw + TARGET_MAX_CENTRE_MW - 1) // TARGET_MAX_CENTRE_MW)
    )
...
def build_materials_summary(
    total_compute_mw: float, total_capex_gbp: float
) -> dict[str, Any]:
    return {
        "assumptions": [
...
def allocate_budget(
    scoped_candidates: pd.DataFrame, constraints: UserConstraints, top_k: int
) -> BudgetPlan:
    ...
    centre_count = min(
        len(candidates),
        recommended_centre_count(requested_compute, budget, median_capex),
    )
    ...
    for _, row in candidates.head(centre_count).iterrows():
        capex = (
            per_centre_compute
            * float(row["estimated_capex_per_mw_gbp"])
            * (1 + CONTINGENCY_RATE)
        )
    ...
    problem = (
        None
        if feasible
        else f"Estimated capex GBP {total_capex:,.0f} exceeds stated budget GBP {budget:,.0f}."
    )
    logger.debug(
