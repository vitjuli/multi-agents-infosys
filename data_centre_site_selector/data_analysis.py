...
def add_production_scores(df: pd.DataFrame, constraints: UserConstraints) -> pd.DataFrame:
    logger.debug("Adding production scores rows=%s workload=%s optimise=%s.", len(df), constraints.workload, constraints.optimisation_choices)
    ...
    gsp_distance = out["nearest_gsp_distance_km"].fillna(out["nearest_gsp_distance_km"].median())
    ...
    out["emissions_intensity_proxy_kgco2e_per_mwh"] = (280 - (out["co2_score_raw"] * 18)).map(lambda v: max(65.0, v))
    out["population_strain_score_raw"] = (
        0.55 * out["water_score_raw"]
        + 0.25 * population.map(lambda v: _linear(v, population.min(), population.max(), invert=True))
        + 0.20 * out["resilience_score_raw"]
    ).map(clamp)
    out["political_favour_score_raw"] = out.apply(lambda row: policy_score(row["region"], row.get("country")), axis=1)
    out["infrastructure_score_raw"] = (
        0.45 * out["energy_score_raw"]
        + 0.25 * out["latency_score_raw"]
        + 0.20 * gsp_distance.map(lambda v: _linear(v, 0, max(20, gsp_distance.quantile(0.9)), invert=True))
        + 0.10 * brownfield.map(lambda v: _linear(v, 0, max(50, brownfield.quantile(0.9))))
    ).map(clamp)
    out["land_use_score_raw"] = (0.75 * out["land_score_raw"] + 0.25 * brownfield.map(lambda v: _linear(v, 0, max(50, brownfield.quantile(0.9))))).map(clamp)
    out["estimated_capex_per_mw_gbp"] = out.apply(estimate_capex_per_mw, axis=1)
    capex = out["estimated_capex_per_mw_gbp"]
    out["cost_score_raw"] = capex.map(lambda v: _linear(v, capex.min(), capex.max(), invert=True))
    ...
    logger.debug(
        ...
        out[["region", "production_score", "co2_score_raw", "population_strain_score_raw", "cost_score_raw"]].head(5).to_dict(orient="records"),
    )
    return out
...
def estimate_capex_per_mw(row: pd.Series) -> float:
    ...
    infrastructure_adjustment = (5.0 - float(row.get("infrastructure_score_raw", 5.0))) * 220_000
...
def filter_for_scope(df: pd.DataFrame, constraints: UserConstraints) -> pd.DataFrame:
    if constraints.region_level == "uk" or not constraints.region_text or constraints.region_text == "UK-wide":
        return df.copy()
    if constraints.region_level == "country":
        return df[df["country"].str.lower() == str(constraints.region_text).lower()].copy()
...
def top_region_records(df: pd.DataFrame, top_k: int) -> list[dict[str, Any]]:
    ...
    records = df.head(top_k)[[key for key in keep if key in df.columns]].to_dict(orient="records")
    return [{key: json_value(row.get(key)) for key in keep if key in row} for row in records]


def nested_search(features: pd.DataFrame, constraints: UserConstraints, top_k: int) -> tuple[pd.DataFrame, list[SearchStage]]:
    scored = add_production_scores(features, constraints)
    logger.debug("Nested search started level=%s region=%s total_candidates=%s.", constraints.region_level, constraints.region_text, len(scored))
    stages = [
        SearchStage(
            level="uk",
            label="UK-wide screening",
            candidate_count=len(scored),
            criteria=["CO2 proxy", "population water/energy strain", "policy favour", "cost", "infrastructure", "land reuse"],
    ...
    if constraints.region_level in {"country", "city"}:
        ...
        if constraints.region_level == "city":
            match = scored[scored["region"].str.lower() == str(constraints.region_text).lower()]
            country_label = match.iloc[0]["country"] if len(match) else constraints.region_text
        country_df = scored[scored["country"].str.lower() == str(country_label).lower()].copy()
        stages.append(
            SearchStage(
                level="country",
                label=f"{country_label} screening",
                candidate_count=len(country_df),
                criteria=["national policy fit", "grid and renewable capacity", "land/planning risk"],
                top_regions=top_region_records(country_df, top_k),
                blocked_reason=None if len(country_df) else f"No cached candidate regions for {country_label}.",
            )
        )
        scoped = country_df
        logger.debug("Country scoped candidates label=%s count=%s.", country_label, len(country_df))
    if constraints.region_level == "city":
        scoped = filter_for_scope(scored, constraints)
        logger.debug("Local scoped candidates label=%s count=%s.", constraints.region_text, len(scoped))
        stages.append(
            SearchStage(
                level="city",
                label=f"{constraints.region_text} site-cluster screening",
                candidate_count=len(scoped),
                criteria=["local infrastructure", "site-level policy eligibility", "cost feasibility", "community strain"],
                top_regions=top_region_records(scoped, top_k),
                blocked_reason=None if len(scoped) else f"No cached candidate region for {constraints.region_text}.",
            )
        )
    scoped = scoped.reset_index(drop=True)
    logger.debug("Nested search complete scoped_count=%s stages=%s.", len(scoped), [stage.label for stage in stages])
