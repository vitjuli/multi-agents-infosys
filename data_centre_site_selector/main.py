...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="UK production data-centre site-selection backend.")
    parser.add_argument("--prompt", "--query", dest="query", default="Find the best UK location for a data centre", help="Natural-language decision prompt.")
    parser.add_argument("--workload", default=None, choices=sorted(WORKLOAD_WEIGHTS), help="Workload profile. If omitted, inferred from the prompt.")
    parser.add_argument("--budget", default=None, help="Budget, for example '£1.2bn' or '500m GBP'.")
    parser.add_argument("--region", default=None, help="UK scope: UK-wide, England, Scotland, Wales, Northern Ireland, or a supported city/cluster.")
    parser.add_argument("--compute-mw", type=float, default=None, help="Target IT compute/load in MW.")
    parser.add_argument("--optimise", action="append", default=None, help="Optimisation priority. Can be repeated, e.g. --optimise co2 --optimise cost.")
    ...
    parser.add_argument("--enable-web-policy", action="store_true", help="Allow the policy agent to use OpenAI web search for current UK policy/grant/tax context.")
    parser.add_argument("--interactive", action="store_true", help="Ask once for clarification when required fields are missing or critics fail.")
...
def main() -> None:
    ...
    if args.interactive and result["site_selection"]["needs_human_input"]:
        ...
        answer = input(f"{result['site_selection']['human_input_prompt']}\nAdditional input: ").strip()
        if answer:
            logger.debug("Received interactive clarification with %d characters.", len(answer))
            result = run_site_selection(
